...
The credentials are used to request an *access token* from the identity provider. The token is cached locally and passed along with each request as a HTTP Authorization header to the Delivery API services. If the authentication request fails then a 401 HTTP status code is returned, and an exception is thrown.
